...
import {CreateDepartment} from "../../forms/CreateDepartmentFrom.tsx";
...
const LineDepartment: FC<Props> = ({deleteFunction, isLoadingDelete, item}) => {
    const {store} = useContext<PStore>(Context);
    const [isEdit, setIsEdit] = useState(false);
    const [newItem, setNewItem] = useState<CreateDepartment>(item);
    const [isLoading, setIsLoading] = useState(false);

    const editDepartment = (id: number) => {
        setIsLoading(true);
        $api.put<ResponseLevelPosition>(`/department/${id}/`, newItem)
            .then((response) => {
