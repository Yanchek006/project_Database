...
import {CreatePosition, ResponsePosition} from "../../forms/CreatePositionFrom.tsx";
...
const LinePosition: FC<Props> = ({deleteFunction, isLoadingDelete, item}) => {
    const {store} = useContext<PStore>(Context);
    const [isEdit, setIsEdit] = useState(false);
    const [newItem, setNewItem] = useState<CreatePosition>(item);
    const [isLoading, setIsLoading] = useState(false);

    const editPosition = (id: number) => {
        setIsLoading(true);
        $api.put<ResponsePosition>(`/position/${id}/`, newItem)
            .then((response) => {
                store.setEntityById(store.positions, id, response.data);
                setIsEdit(false);
            })
            .catch(() => setNewItem(item))
