...
import {ResponseState} from "../ShowState.tsx";
import $api from "../../../../../../http";
import {CreateState} from "../../forms/CreateStateForm.tsx";
...
const LineState: FC<Props> = ({deleteFunction, item, isLoadingDelete}) => {
    const {store} = useContext<PStore>(Context);
    const [isEdit, setIsEdit] = useState(false);
    const [newItem, setNewItem] = useState<CreateState>(item);
    const [isLoading, setIsLoading] = useState(false);
    const editState = (id: number) => {
        setIsLoading(true);
        $api.put<ResponseState>(`/state/${id}/`, newItem)
            .then((response) => {
                store.setEntityById(store.states, id, response.data);
                setIsEdit(false);
            })
